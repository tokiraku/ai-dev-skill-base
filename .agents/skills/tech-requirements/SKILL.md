...
3. **`copilot-instructions.md` への反映** — 両ツールで共有できるコンテキスト宣言
...
共通コンテキストを **単一の中間ファイル** に集約し、各ツール固有のファイルからそれを参照する。
これにより CLAUDE.md と copilot-instructions.md の内容が二重管理にならない。
...
graph LR
    ...
    CP["copilot-instructions.md (GitHub Copilot)"]
    ...
    style CP  fill:#d0e8ff,stroke:#4a90d9,color:#000
```

> **重要**: `CLAUDE.md` と `.github/copilot-instructions.md` はワークスペース全体に影響するため、必ずユーザーの承認を得てから書き込む。

---

#### 8-1. `CLAUDE.md` への反映（Claude Code 用）

Claude Code は `CLAUDE.md` 内の `@パス` 構文でファイルをインポートできる。
Step 6・7 で生成した2ファイルを直接インポートする。
...
- 存在しない場合: 新規作成する（Write）

---

#### 8-2. `.github/copilot-instructions.md` への反映（GitHub Copilot 用）

Copilot はネイティブのインポート構文を持たないため、2ファイルを参照するよう **テキストで指示** する。
...
手順:
- `.github/copilot-instructions.md` が存在する場合: Read して末尾に追記する（Edit）
- 存在しない場合: `.github/` ディレクトリごと新規作成する（Write）
...
| GitHub Copilot（参照指示） | .github/copilot-instructions.md |
...
- `CLAUDE.md`・`.github/copilot-instructions.md` への書き込みは**必ずユーザーの承認を得てから**実行する
